...
(function($){
	...
	_view = {
		addAddBtn: function(setting, node) {
			if (node.editNameFlag || $$(node, consts.id.ADD, setting).length > 0) {
				return;
			}
			
			if(setting.add === undefined){
				return;
			}
			var aObj = $$(node, consts.id.A, setting),
			editStr = "<span class='" + consts.className.BUTTON + " add' id='" + node.tId + consts.id.ADD + "' title='"+tools.apply(setting.edit.addTitle, [setting.treeId, node], setting.edit.addTitle)+"' treeNode"+consts.id.ADD+" style='display:none;'></span>";
			aObj.append(editStr);

			$$(node, consts.id.ADD, setting).bind('click',
				function() {
					tools.apply(setting.add.click, [setting.treeId, node]);
					return false;
				}
				).show();
		},
		addEditBtn: function(setting, node) {
			...
			$$(node, consts.id.EDIT, setting).bind('click',
				function() {
					tools.apply(setting.edit.click, [setting.treeId, node]);
					return false;
				}
				).show();
		},
		addRemoveBtn: function(setting, node) {
			...
			$$(node, consts.id.REMOVE, setting).bind('click',
				function() {
					tools.apply(setting.remove.click, [setting.treeId, node]);
...
//					setting.treeObj.trigger(consts.event.REMOVE, [setting.treeId, node]);
		...
		removeAddBtn: function(setting, node) {
			$$(node, consts.id.ADD, setting).unbind().remove();
		},
		...
		removeTreeDom: function(setting, node) {
			node.isHover = false;
			view.removeAddBtn(setting, node);
